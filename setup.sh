...
if [ "$(printf '%s\n' "$required_version" "$python_version" | sort -V | head -n1)" = "$required_version" ]; then
    echo "[OK] Python $python_version (OK)"
else
    echo "[ERROR] Python $python_version is too old. Please install Python 3.9 or later."
    exit 1
fi
...
if [ ! -f "config.yaml" ]; then
    echo "[WARN] No config.yaml found. Using default configuration."
...
" 2>/dev/null; then
    ...
    echo "[WARN] Some modules failed to import. Check error messages above."
fi
